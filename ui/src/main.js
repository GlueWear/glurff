...
import { aimShot, validShot, SHOT_COOLDOWN, STUN_MS } from 'lib/slingshot';
 ...
 */
import { initApi, our } from 'lib/api';
import { initNoltbook, nb, COMMONS_NOTE, visiblePeers, displayName, onChange, setDial, updateActiveCount, holdHistory, releaseHistory, inGlurff } from 'lib/noltbook';
import * as G from 'lib/glurff';
import { Game } from 'world/game';
import { COMMONS, roomById, regionAt } from 'world/places';
...
const game = new Game(document.getElementById('stage'), {
  onMove: throttle(reportPosition, 1000 / G.MOVE_HZ),
  ...
  onRoomChange: (room, from) => {
    ...
    events.clear();
    R.enterRoom(room);
    /* Walking in pulls the camera close enough to read the room; walking out
     * pulls all the way back so you can see where everyone is. */
    if (room !== 0) { if (game.zoom < 2) game.setZoom(2); }
    else game.setZoom(0.5);
    hud.setRoom(room);
...
const hud = new Hud(hudRoot, { onGoBuilder: () => builder.toggle(), events });
...
/* Messages sit in the top right and carry the unread dot; the profile card
 * opens over everything. They are wired to each other so a DM button on a card
 * opens the conversation, and the card can be reached back from inside it. */
const dmRoot = document.createElement('div');
document.body.appendChild(dmRoot);
const cardRoot = document.createElement('div');
document.body.appendChild(cardRoot);
const card = new ProfileCard(cardRoot, { onOpenDm: (ship) => dms.openWith(ship) });
const dms = new Dms(dmRoot, { onShowProfile: (ship) => card.open(ship) });

/* Top left: who is here with you -- the room you are in, or the commons -- and
 * who hosts it. The same people the world draws. */
const membersRoot = document.createElement('div');
document.body.appendChild(membersRoot);
...
function reportPosition(self, force = false) {
  ...
  movement?.moved({ x: self.x, y: self.y, dir: self.dir, host: R.rooms.host ?? null }, force);
  /* Proximity: who is close enough to talk to, and how loud they are. */
...
const shots=new Map(), shooterTimes=new Map();
function receiveShot(who,event) {
  const origin=who===our?game.self:state.peers.get(who)?.spot;
  const now=performance.now();
  if(!validShot(event,origin)||shots.has(who+'/'+event.id)||now-(shooterTimes.get(who)??-Infinity)<SHOT_COOLDOWN-100)return;
  for(const [id,t] of shots)if(now-t>60000)shots.delete(id);
  if(shots.size>500)return;
  shots.set(who+'/'+event.id,now);shooterTimes.set(who,now);
  const to=aimShot(event.from,event.to,(x,y)=>game.solidAt(x,y));
  game.projectile(event.from,to,()=>{
    for(const [ship,p] of state.peers)if(Math.hypot(p.spot.x-to.x,p.spot.y-to.y)<.8)game.soak(ship);
    if(who!==our && Math.hypot(game.self.x-to.x,game.self.y-to.y)<.8 && performance.now()>(game.stunnedUntil??0)) {
      game.stunnedUntil=performance.now()+STUN_MS;
      game.soak(our);
    }
  });
}
game.mount.addEventListener('contextmenu',e=>{
  if(!game.world)return;
  e.preventDefault();
  const now=performance.now();if(now-lastShot<SHOT_COOLDOWN)return;
  lastShot=now;
  const from={x:game.self.x,y:game.self.y},to=aimShot(from,game.worldPoint(e.clientX,e.clientY),(x,y)=>game.solidAt(x,y));
  const event={kind:'shot',id:crypto.randomUUID(),from,to};
  receiveShot(our,event);
  for(const ship of state.peers.keys())G.sendPresence(ship,event).catch(()=>{});
});
...
function applyPresence() {
  const peers = pendingPeers;pendingPeers = null;
  nb.discovered=Object.fromEntries([...peers].map(([ship,p])=>[ship,{hops:p.path.length-1,at:p.at}]));
...
const presence = createPresence({
  ...
  snapshot:()=>({stamp:Date.now(),spot:{place:COMMONS,x:Math.round(game.self.x*G.SUB),y:Math.round(game.self.y*G.SUB),dir:game.self.dir},rev:state.rev,host:R.rooms.host??null,mv:movement?.announce()??null}),
  send:G.sendPresence,
...
setInterval(()=>{
  ...
  if(presenceStarted && movement) {
    ...
    if(next!==claimSignature){claimSignature=next;presence.publish();}
  }
...
dial.className = 'dial';
dial.innerHTML = 'Dial <select aria-label="Discovery dial"><option value="0">0</option><option value="1">1</option><option value="2">2</option><option value="3">3</option></select><span role="status"></span>';
dms.addTool(dial);   //  top right, right before the search button
const dialSelect = dial.querySelector('select');
onChange(() => { dialSelect.value = String(nb.dial); }, c => c.field === 'dial');
dialSelect.onchange = async () => {
  dialSelect.disabled = true;
  try { await setDial(dialSelect.value); dial.querySelector('span').textContent=''; }
  catch { dialSelect.value=String(nb.dial); dial.querySelector('span').textContent='Could not change dial'; }
  finally { dialSelect.disabled=false; }
};


function onWorldFact(name, p, fromPresence=false) {
  if(name==='presence-event') {
    try {const event=JSON.parse(p.body);if(event.kind?.startsWith('call-')){R.receiveCallEvent(p.who,event);}else if(event.kind==='shot'){if(state.peers.has(p.who))receiveShot(p.who,event);}else presence.receive(p.who,event);}catch(e){console.warn('Invalid presence event');}
    return;
  }
  ...
  switch (name) {
    ...
    case 'refused':
      ...
      break;
    case 'splashed':
      /* Someone got us. Everyone who can see us sees it too, because they get
       * the same event from the sender. */
      if (worldReady) game.soak(our);
      break;
    default:
      break;
  }
